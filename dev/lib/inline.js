    ...
    return inlineMap.map(({ name, match, keyWord }) => {
        ...
        const resolve = (events, context) => {
            ...
            for (let index = events.length - 1; index > -1; index--) {
                ...
                const inside = resolveAll(context.parser.constructs.insideSpan.null, events.slice(open + 1, close), context);
                const exit = [
                    ['exit', text, context],
                    ['exit', event, context]
                ];
